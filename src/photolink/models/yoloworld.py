...
class YOLOWorld:
    ...
    def __init__(self, path, conf_thres=0.3, iou_thres=0.5):
        ...
        logger.info(f"Using {self.session.get_providers()}")
    ...
    def detect_objects(self, image, class_embeddings):
        ...
        if class_embeddings.shape[1] != self.num_classes:
            raise ValueError(
                f"Number of classes in the class embeddings should be {self.num_classes}"
            )
...
class DetectionDrawer:
    ...
    def draw_detections(self, image, boxes, scores, class_ids, mask_alpha=0.3):
        ...
        size = min([img_height, img_width]) * 0.0002
...
if __name__ == "__main__":
    ...
    model_path = str(Path("assets/weights/yoloworld.onnx"))
    embed_path = str(Path("assets/weights/grad_embedding.onnx"))

    # Load class embeddings
    class_embeddings, class_list = read_class_embeddings(embed_path)
    logger.info(f"Detecting classes: {class_list}")
    ...
    img_url = str(Path("demo/USFUCS24-C1P1-00010.JPG"))
    ...
    cv2.imwrite("demo/detected_humans.png", combined_img)
    logger.info("Detection saved to detected_humans.png")
